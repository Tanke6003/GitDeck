...
describe('hunkService', () => {
  ...
  it('trocea el diff en un hunk por cada zona de cambios', async () => {
    ...
    const diff = await getFileHunks(fx.dir, 'f.txt')
  ...
  it('quita del staging un solo hunk sin tocar el archivo en disco', async () => {
    ...
    const cached = await getFileHunks(fx.dir, 'f.txt', true)
  ...
  it('devuelve null cuando el archivo no tiene cambios', async () => {
    expect(await getFileHunks(fx.dir, 'f.txt')).toBeNull()
  })
  ...
  it('conserva el marcador de "sin salto de linea al final"', async () => {
    ...
    const diff = await getFileHunks(fx.dir, 'n.txt')
